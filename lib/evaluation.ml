...
and eval (env : env) (tm : term) : value =
  ...
  | App (f, a) -> eval_app (eval env f) (eval env a)
...
let rec force (v : value) : value =
  ...
  | VNe (NMeta m, sp) -> begin
      match lookup_meta m with
      | Solved t -> force (List.fold_right (fun x acc -> eval_app acc x) sp t)
      | _ -> v
    end
  | _ -> v
...
let rec quote (lvl : level) (value : value) : term =
  ...
  | VNe (head, spine) ->
      let f h = List.fold_right (fun v acc -> App (acc, quote lvl v)) spine h in
      begin match head with
      | NVar x -> f (Var (index_of_level lvl x))
      | NMeta m -> f (Meta m)
      end
  | VLam (name, f_clos) -> Lam (name, quote (next_level lvl) (f_clos $$ vvar lvl))
  | VPi (name, a_ty_val, f_clos) ->
      Pi (name, quote lvl a_ty_val, quote (next_level lvl) (f_clos $$ vvar lvl))
  | VUniverse -> Universe
