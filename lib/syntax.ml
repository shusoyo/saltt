...
let rec strip (t : term) : term =
  match t with Ann (t_inner, _) -> strip t_inner | _ -> t
...
let rec close_term_aux (t : term) (x : name) (n : int) : term =
  ...
  | Ann (t_inner, ty) -> Ann (close_term_aux t_inner x n, close_term_aux ty x n)
...
module NamedAst = struct
  ...
  let rec of_term (t : term) : n_term =
    ...
    | Lam body ->
        let x, opened = unbind "x" body in
        Lam (x, of_term opened)
    | TyPi (a, b) ->
        let a' = of_term a in
        let x, opened = unbind "x" b in
